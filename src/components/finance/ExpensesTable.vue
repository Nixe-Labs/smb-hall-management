...
const editing = ref<Partial<Expense>>({})
...
function openAdd() {
  editing.value = { amount: 0, category_id: '', description: '' }
...
async function save() {
  ...
  try {
    ...
    if (editing.value.id) {
      const { error } = await supabase.from('expenses').update(payload).eq('id', editing.value.id)
...
async function confirmDelete() {
  ...
  deleting.value = true
  try {
    const { error } = await supabase.from('expenses').delete().eq('id', confirmDeleteId.value)
    if (error) throw error
    emit('updated')
  } catch (e: any) {
...
const total = () => props.expenses.reduce((s, i) => s + i.amount, 0)
</script>
      ...
      <table class="table">
        ...
        <tbody>
          <tr v-if="expenses.length === 0">
            <td :colspan="(canEdit || canDelete) ? 4 : 3" style="text-align:center;color:var(--ash);padding:40px">No expenses recorded.</td>
          </tr>
          <tr v-for="item in expenses" :key="item.id">
            <td style="font-weight:600">{{ categoryName(item.category_id) }}</td>
            <td style="color:var(--ash)">{{ item.description ?? '—' }}</td>
            <td style="text-align:right;font-family:var(--font-display);font-weight:600;color:var(--signal-red)">−{{ formatCurrency(item.amount) }}</td>
        ...
        <tfoot v-if="expenses.length > 0">
          <tr>
            ...
            <td style="text-align:right;font-family:var(--font-display);font-weight:700;color:var(--signal-red)">−{{ formatCurrency(total()) }}</td>
    ...
    <Teleport to="body">
      <div v-if="showModal" class="smb-modal-overlay" @click.self="showModal = false">
        <div class="smb-modal">
          <div class="smb-modal-header">
            <h3 class="t-h3">{{ editing.id ? 'Edit' : 'Add' }} Expense</h3>
          ...
          <div class="smb-modal-body">
            <div class="form-stack">
              ...
              <div>
                ...
                <input class="input" v-model="editing.description" placeholder="Optional description" />
              </div>
            </div>
          </div>
      ...
      <div v-if="confirmDeleteId" class="smb-modal-overlay" @click.self="confirmDeleteId = null">
        <div class="smb-modal" style="max-width:400px">
          <div class="smb-modal-header">
            <h3 class="t-h3">Delete expense?</h3>
          </div>
          <div class="smb-modal-body">
            <p style="color:var(--ash);line-height:1.6">This will permanently remove this expense. This action cannot be undone.</p>
          </div>
          <div class="smb-modal-footer">
