...
async function toggleActive(item: ExpenseCategory) {
  await supabase.from('expense_categories').update({ is_active: !item.is_active }).eq('id', item.id)
  await fetchCategories()
}
...
<template>
  <div>
    <div class="flex items-center gap-4 mb-6">
      ...
      <Button label="Add Category" icon="pi pi-plus" size="small" class="ml-auto" @click="openAdd" />
    </div>
