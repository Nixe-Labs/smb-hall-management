    ...
    <div v-else>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        <div class="bg-white rounded-lg shadow p-6">
          ...
          <div class="text-2xl font-bold text-green-600 mt-1">{{ formatCurrency(stats.total_revenue) }}</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
          <div class="text-sm text-gray-500">Total Expenses</div>
          <div class="text-2xl font-bold text-red-600 mt-1">{{ formatCurrency(stats.total_expenses) }}</div>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
          <div class="text-sm text-gray-500">Net Profit</div>
          <div class="text-2xl font-bold mt-1" :class="stats.net_profit >= 0 ? 'text-green-600' : 'text-red-600'">
            {{ formatCurrency(stats.net_profit) }}
          </div>
        </div>
        <div class="bg-white rounded-lg shadow p-6">
          <div class="text-sm text-gray-500">Total Bookings</div>
          <div class="text-2xl font-bold text-blue-600 mt-1">{{ stats.total_bookings }}</div>
